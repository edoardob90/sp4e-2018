...
std::shared_ptr<Particle> MaterialPointsFactory::createParticle() {
  return std::make_shared<MaterialPoint>();
}
...
void MaterialPointsFactory::createDefaultComputes(Real timestep) {
  ...
  // Use the setters here since we need them for the binding anyway
  compute_temp->setConductivity(1.);
  compute_temp->setL(2.);
  compute_temp->setCapacity(1.);
  compute_temp->setDensity(1.);
